...
#dplyr
names(sales) ## in the dataset what are headers
...
names(sales) ### correct variable name
...
library(dplyr)
sales %>% count(custname,sort=TRUE) %>% head(n=5) #### sales dataframe is piped the count command then goes through the custname list **sort=true means??
#Ans: CHIZ BROS INC most loyal with 253 times purchase
...
sales %>% group_by(custname) %>%  summarize(Revenue = sum(revenue)) %>% arrange(desc(Revenue)) %>% head(n=3) ## pipeline includes just these data
