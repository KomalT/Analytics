...
?women
plot(women)
plot(women, type='p', pch=17) ## p points ** pch plotting symbol
...
plot(women)
abline(lm(women$weight ~ women$height),xlim=c(0,100), ylim=c(min(women$weight)-0, 200), col='red', lty=2, lwd=4)
...
#barplot
barplot(x,col=1:4) ## are not continuous
...
head(mtcars)
pairs(mtcars)
options(digits=4) ### different from trunc ceiling floor
